...
def compute_node_weight_sum(edge_path: str, output_dir: str):
    ...
    for f in tqdm(files, desc="Node SUM"):
        ...
        in_w.columns = ["node", "in_weight"]

        out_w = df.groupby("From_name", dropna=False)["value"].sum().reset_index()
        out_w.columns = ["node", "out_weight"]

        node_w = pd.merge(in_w, out_w, on="node", how="outer").fillna(0.0)
        node_w["avg_weight"] = (node_w["in_weight"] + node_w["out_weight"]) / 2.0

        out_file = os.path.join(output_dir, f.replace("_edges.csv", "_node_weights.csv"))
        node_w.to_csv(out_file, index=False)


def compute_node_weight_avg(edge_path: str, output_dir: str):
...
def main():
    results_dir = "./CellTOSG_analysis_results/disease/Crohn_disease/gat/epoch_50_3_0.0005_2025_20250916_133113"
    ...
    node_sum_out = os.path.join(results_dir, "node_weights_sum")
    node_avg_out = os.path.join(results_dir, "node_weights_avg")

    compute_node_weight_sum(edge_path=edge_sum_dir, output_dir=node_sum_out)
    compute_node_weight_avg(edge_path=edge_sum_dir, output_dir=node_avg_out)
